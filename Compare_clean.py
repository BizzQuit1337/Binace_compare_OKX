...
def compare(dict_okx, dict_binance):
    ##Variables to keep track of lists
    list_binance = []
    list_okx = []
    match_list = []
    compare = []
    ...
    ##Creating the lists of just numbers
    ...
    ##Match aspects that are the same 
    ...
    ##Compare from the list
    for i in match_list:
        data = {
            'symbol binance':dict_binance[i[0]]['symbol'],
            'symbol okx':dict_okx['instId'][i[1]],
    ...
    ##The comparison is a bit unfinished need to minimise and re work the comparison.
